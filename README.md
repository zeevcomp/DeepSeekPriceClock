...
- 💰 **Complete price table** per 1M tokens (USD) for **every model on DeepSeek's official pricing page** (currently `deepseek-flash` and `deepseek-v4-pro`): **both off-peak and peak prices** for every metric (cache-hit input, cache-miss input, output). The model list, names and prices are **discovered automatically** from the page — new or renamed models appear on their own. The row matching the **current** price state is highlighted; the inactive one stays dim
...
4. The **analog clock is the default** — its thin frame glows **green off-peak / amber at peak**, matching the current price state. Click **English / עברית** to switch language or **Digital** for the big digital clock.
