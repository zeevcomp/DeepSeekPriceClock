...
- 🕰️ **Analog clock (default)** — a polished, pre-rendered face with silky-smooth hands (≈30 fps sweep) and a **thin, elegant neutral frame**. **Only the expensive hours are marked in color**: amber arcs show the peak windows on the dial (converted to your local time), and a small dot at the current hour turns amber while a peak window is active
...
4. The **analog clock is the default** — **amber arcs on the dial mark the expensive (peak) hours** in your local time (weekends have none); the small dot at the current hour is amber only while peak is active. Click **English / עברית** to switch language or **Digital** for the big digital clock.
